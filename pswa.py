...
if __name__ == '__main__':
    ...
    parser.add_argument("--pswa", action="store_true", default=True, help="swa usage flag (default: off)")
    parser.add_argument(
        "--swa_start",
        type=float,
        default=160,
        metavar="N",
        help="SWA start epoch number (default: 160)",
    )
    parser.add_argument(
        "--pswa_start",
        type=float,
    ...
    parser.add_argument('--P', type=int, default=20, help='model recording period (default: 10)')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (
            ...
            if (epoch + 1 - args.pswa_start) % args.P == 0:
        ...
        if (
            ...
            train_res = utils.train_epoch(loaders['train'], model, criterion, optimizer, lr_schedule=0.01, cuda=use_cuda)
            train_loss.append(train_res["loss"])
            test_res = utils.eval(loaders['test'], model, criterion, cuda=use_cuda)
            utils.moving_average(swa_model, model, 1.0/(swa_n + 1))
            swa_n += 1
        ...
        if (epoch + 1) % args.save_freq == 0:
            ...
            if args.swa:
                utils.save_checkpoint(
                    args.dir, epoch + 1, name="swa", state_dict=swa_model.state_dict()
                )
