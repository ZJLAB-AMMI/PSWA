...
def train_and_test(data, dataset_root, config):
    ...
    # 测试集准确率
    test_acc, swa_acc = pipeline.predicts(prep_dataset, 'test')
    print('[{}] Test Accuracy: {:.3f}\n'.format(data.upper(), test_acc))
    print('[{}] SWA Test Accuracy: {:.3f}\n'.format(data.upper(), swa_acc))
...
if __name__ == '__main__':
    ...
    # Citeseer Test Accuracy: 0.719

    # 使用Pubmed数据集训练和测试模型
    train_and_test('pubmed', dataset_root, config)
    # Pubmed Test Accuracy: 0.782
