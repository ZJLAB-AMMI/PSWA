...
def train_and_test(data, dataset_root, config):
    ...
    test_loss, swa_loss, test_acc, swa_acc = pipeline.predicts(dataset, 'test')
    print('[{}]-[TestLoss:{:.4f}]-[TestAcc:{:.4f}]\n'.format(
        data.upper(), test_loss, test_acc))
    print('[{}]-[SWA TestLoss:{:.4f}]-[SWA TestAcc:{:.4f}]\n'.format(
        data.upper(), swa_loss, swa_acc))
